/*
	Author : redarch
	Created On : 21/01/2021 19:10:55
*/
...
void solve(){
	...
	int a, b, c, d;
	tie(a, b, c, d) = mt(-1, -1, -1, -1);
	ll s1 = -1, s2 = -1;
	for(int i = 0 ; i < n; i++){
		for(int j = 0; j < n; j++){
			ll cur = d1[i + j] + d2[i -j + 2000] - arr[i][j];
			if(i % 2 == j % 2){
				if(cur > s1) tie(s1, a, b) = mt(cur, i, j);
			}
			else{
				if(cur > s2) tie(s2, c, d) = mt(cur, i, j);
			}
		}
	}
	print(s1 + s2);
