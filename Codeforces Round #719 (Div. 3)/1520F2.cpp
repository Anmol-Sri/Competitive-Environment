...
// (200000)/10 = 20000 segments. remaining queries = 60000 - 20000 = 40000 = 4e4 
// for 1e4 queries which will be of at most 10 elements
// log2(10) = 3.3 ~= 4
// Therefor at max we will have 1e4 * 4 = 4e4 queries which is <= 4e4(remaining queries).
const int mxN = 2e5 + 10;
vector < ll > store(mxN, 0);
...
void solve(){
	...
	while(t--){
		if(ok){
			ok = false;
			for(ll i = 0; i < mxrange; i++){
				ll l = i * 10 + 1, r = (i + 1) * 10;
				r = min(r, n);
				store[i] = r - l + 1 - ask(l, r);
			}
		}
		ll l = 1, r = 10;
		for(ll i = 0; i < mxrange; i++){
