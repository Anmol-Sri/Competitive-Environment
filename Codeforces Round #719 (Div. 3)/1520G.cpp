...
void solve(){
    ...
    auto bfs = [&](pair < int, int > p){ // dijkstra_shortest path on 2d array.
        ...
        while(!q.empty()){ 
            ...
            for(int i = 0; i < 4; i++){
                ...
                if(a[x1][y1] == -1) continue; // if -1 movement not possible.
                if(d[x1][y1] <= d[x][y] + 1) continue; // change value / add to queue -> only if it decreases the movement time.
                d[x1][y1] = d[x][y] + 1;
                q.emplace(x1, y1);
            }
        }
    ...
    auto d1 = bfs({0, 0}); // minimum movement time starting from [0, 0]
    auto d2 = bfs({n - 1, m - 1}); // minimum movement time starting from [n - 1, m - 1]
    ll ans = INFL;
    if(d1[n - 1][m - 1] != INF) ans = d1[n - 1][m - 1] * w; // minimum time to reach [n - 1, m - 1] from [0, 0] -> for each step cost is w
    ll mn1 = INFL, mn2 = INFL;
    for(ll i = 0; i < n; i++) for(ll j = 0; j < m; j++) if(a[i][j] > 0 && d1[i][j] != INF) mn1 = min(mn1, a[i][j] + d1[i][j] * w);
    // finding portal closest to [0, 0]
    for(ll i = 0; i < n; i++) for(ll j = 0; j < m; j++) if(a[i][j] > 0 && d2[i][j] != INF) mn2 = min(mn2, a[i][j] + d2[i][j] * w);
    // finding portal closest to [n - 1, m - 1]
    ans = min(ans, mn1 + mn2);
    if(ans > 1e16) ans = -1;
    print(ans);
}
