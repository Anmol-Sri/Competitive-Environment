...
void init(){
    ...
    for(ll i = 2; i < mxN; i++){
		for(auto x : primes){
			if(x == i){
				s[i].clear(); s[i].insert(x); break;
			}
			else if(x < i){
				if(s[i].size() == 0 || (s[i].size() > s[i - x].size() + 1 && s[i - x].size() != 0)){
					s[i].clear();
					for(auto y : s[i - x]) s[i].insert(y);
					s[i].insert(x);
				}
			}
		}
...
void solve(){
	ll n; cin >> n;
    if(s[n].size() == 0) cout << -1 << nl;
    else cout << s[n].size() << nl;
}
