...
ll arr[505][505], c[505][505], R[505], C[505];
ll parent[1100];
ll find(ll x){return parent[x] == x ? x : parent[x] = find(parent[x]); }
bool join(ll x, ll y){ if((x = find(x)) == (y = find(y))) return false; parent[x] = y; return true; }
void solve(){
    ll n; cin >> n;
    ...
    for(ll i = 0; i < n; i++) for(ll j = 0; j < n; j++) if(arr[i][j] < 0) {ans += c[i][j]; edges.pb({c[i][j], i, j + n});}
    sort(all(edges)); reverse(all(edges));
    for(ll i = 0; i < 2 * n; i++) parent[i] = i;
