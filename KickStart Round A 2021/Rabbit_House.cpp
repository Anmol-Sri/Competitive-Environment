...
void calc(ll i, ll j){
    if(i - 1 >= 0){
        if(abs(arr[i][j] - arr[i - 1][j]) > 1){
            ans += abs(arr[i][j] - arr[i - 1][j]) - 1;
            if(arr[i][j] < arr[i - 1][j]) arr[i][j] += abs(arr[i][j] - arr[i - 1][j]) - 1;
            else arr[i - 1][j] += abs(arr[i][j] - arr[i - 1][j]) - 1;
            calc(i, j);
            calc(i - 1, j);
        }
    }
    ...
    if(j - 1 >= 0){
        if(abs(arr[i][j] - arr[i][j - 1]) > 1){
            ans += abs(arr[i][j] - arr[i][j - 1]) - 1;
            if(arr[i][j] < arr[i][j - 1]) arr[i][j] += abs(arr[i][j] - arr[i][j - 1]) - 1;
            else arr[i][j - 1] += abs(arr[i][j] - arr[i][j - 1]) - 1;
            calc(i, j);
            calc(i, j - 1);
        }
    }
...
void solve(){
    ...
    // for(ll i = 0; i < r; i++) for(ll j = 0; j < c; j++) g[i][j] = 0;
    ans = 0;
    // auto calc = [&](ll i, ll j){
    //     if(i - 1 >= 0){
    //         if(abs(arr[i][j] - arr[i - 1][j]) > 1){
