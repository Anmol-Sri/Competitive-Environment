...
bool check(vector < bool > used){
    for(ll i = 0; i < n; i++) if(!used[i]) return false;
    return true;
}
void recur(vector < ll > cur, vector < bool > used){
    if(cur.size() > n) return;
    if(check(used)){
        if(cur.size() == n) {ok = true; return;}
        return;
    }
    if(cur.size() == 1){
        for(ll i = 0; i < n; i++){
...
void solve(){
	cin >> n;
    a.resize(n); read(a); sort(all(a));
    vector < ll > cur; cur.pb(0);
    vector < bool > used(n, false);
    recur(cur, used);
    if(ok) print("YES"); else print("NO");
    // for(ll i = 0; i < n; i++){
    //     set < ll > cur;
    //     cur.insert(0); cur.insert(a[i]);
    //     for(ll j = 0; j < n; j++){
    //         if(j == i) continue;
