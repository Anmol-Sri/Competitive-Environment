...
void solve(){
    ...
    vector < ll > dif(n - 1);
    for(ll i = 1; i < n; i++) dif[i - 1] = arr[i] - arr[i - 1];
    auto calc = [](vector < ll > d){
        ll n = d.size();
        ll ans = 0;
        for(ll i = 0, j = 0; i < n; i = j){
            ...
            if(j < n - 1 && d[j] + d[j + 1] == 2 * d[i]){
                // for the array with vals : 1 3 5 6 9 11 13 15 17...
                // for e.g. if dif array is : 2 2 1 3 2 2 2 2 
                // for this case we can take till | modify 
                ll end = j + 2;
    ...
    ll ans = calc(dif);
    reverse(all(dif)); ans = max(ans, calc(dif));
    print(ans);
}
