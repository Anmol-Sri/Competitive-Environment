...
vector < pair < ll, pair < ll, ll > > > e[mxN];
ll res[mxN];
vector < pair < ll, ll > > arr[mxN];
segtree < gcd_t > st;

void dfs(ll v, ll p){
	for(auto x : arr[v]) res[x.second] = st.query(0, x.first).val;
	for(auto x : e[v]){
		if(x.first != p){
			st.update(x.second.first, x.second.first, x.second.second);
			dfs(x.first, v);
			st.update(x.second.first, x.second.first, -x.second.second);
		}
	}
