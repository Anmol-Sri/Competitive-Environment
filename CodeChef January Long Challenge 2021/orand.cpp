...
void solve(){
	INT N, M; cin >> N >> M;
	set < INT > s; stack < INT > st; vector < INT > arr1(N), arr2(M); read(arr1); read(arr2);
	st.push(0);
	while(st.size() > 0){
		INT T = st.top();
		st.pop();
		for(INT i = 0; i < N; i++) if(s.find(OR(T, arr1[i])) == s.end()) s.insert(OR(T, arr1[i])), st.push(OR(T, arr1[i]));
		for(INT i = 0; i < M; i++) if(s.find(AND(T, arr2[i])) == s.end()) s.insert(AND(T, arr2[i])), st.push(AND(T, arr2[i]));
	}
	print(sz(s));
}
