...
void solve(){
	ll n, m; cin >> n >> m;
	vector < ll > arr(n); read(arr); sort(all(arr));
	set < ll > s; s.insert(arr[n - 1]);
	ll tot = arr[n - 1];
	ll cnt = -1;
	for(int i = n - 2; ~i; i--){
		tot += arr[i];
		set < ll > temp;
		for(auto x : s){
			temp.insert(x + arr[i]); temp.insert(arr[i]); temp.insert(x);
			if(x + arr[i] >= m && tot - x - arr[i] >= m){ cnt = n - i; break; }
			if(arr[i] >= m && tot - arr[i] >= m){ cnt = n - i; break; }
		}
		if(cnt != -1) break;
		s = temp;
	}
