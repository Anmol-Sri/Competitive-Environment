...
const int LIM = 1e6+10; // change the limit according to the question
...
void solve(){
	ll n, m, k; cin >> n >> m >> k;
    vector < vector < ll > > dp(n, vector < ll > (m, INFL));
    ...
    auto calc = y_combinator([&](auto calc, ll x, ll y, ll step) -> void{
        if(a[x][y] == 'X'){
            dp[x][y] = min(dp[x][y], step);
            return;
        }
        if(check(x + 1, y + 2)) calc(x + 1, y + 2, step + 1);
        if(check(x + 1, y - 2)) calc(x + 1, y - 2, step + 1);

        if(check(x + 2, y + 1)) calc(x + 2, y + 1, step + 1);
        if(check(x + 2, y - 1)) calc(x + 2, y - 1, step + 1);
