/*
	Author : redarch
	Created On : 14/01/2021 16:12:29
*/
...
void solve(){
	int n, m; cin >> n >> m;
	map < string, int > mp; map < int, string > inv;
	vector < int > ans;
	bool dp[20][20] = {false};
	auto check = [&](vector < int > v){
		if(sz(v) <= sz(ans)) return false;
		for(auto x : v){
			for(auto y : v){
	...
	for(int i = 0; i < m; i++){
		string a, b; cin >> a >> b;
		dp[mp[a]][mp[b]] = dp[mp[b]][mp[a]] = true;
	}
	for(int i = 1; i < (1 << n); i++){
		vector < int > temp;
		for(int j = 0; j < n; j++){
	...
	sort(all(ans), [&](int a, int b){ return inv[a] < inv[b]; });
	print(sz(ans));
	for(auto x : ans) print(inv[x]);
}
