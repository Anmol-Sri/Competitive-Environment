...
void solve(){
    ll r, c, x; cin >> r >> c >> x;
    vector < vector < ll > > arr1(r, vector < ll > (c));
    vector < vector < ll > > arr2(r, vector < ll > (c));
    for(ll i = 0; i < r; i++) read(arr1[i]);
    for(ll i = 0; i < r; i++) read(arr2[i]);
    // db2d(arr1); db2d(arr2);
    for(ll i = 0; i < r; i++){
        for(ll j = 0; j < c - x + 1; j++){
            ll cur = arr1[i][j] - arr2[i][j];
            for(ll z = j; z < j + x; z++) arr1[i][z] = arr1[i][z] - cur;
        }
    }
    for(ll i = 0; i < c; i++){
        for(ll j = 0; j < r - x + 1; j++){
            ll cur = arr1[j][i] - arr2[j][i];
            for(ll z = j; z < j + x; z++) arr1[z][i] = arr1[z][i] - cur;
        }
    }
    bool ok = true;
    for(ll i = 0; i < r; i++) for(ll j = 0; j < c; j++) if(arr1[i][j] != arr2[i][j]){ok = false; break; }
