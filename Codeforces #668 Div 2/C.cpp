...
void solve(){
	...
	cin >> n >> k;
	string s; cin >> s;
	vector<bool> one(k, false), zero(k, false);
	for(int i=0; i<n; i++){
		if(s[i]=='1') one[i%k]=true;
		if(s[i]=='0') zero[i%k]=true;
	}
	int o=0, z=0;
	bool ok = true;
	for(int i=0; i<k; i++) {if(one[i]==true && zero[i]==true) ok=false; if(one[i])o++; if(zero[i])z++;}
	if(!ok||2*o>k||2*z>k){cout << "NO\n"; return ;}
	cout << "YES\n";
}
