...
void solve(){
	...
	while(true){
		...
		n -= p;
		c++; // number of digits
		p = 9 * power(10, c - 1) * c; // p = number of index's that are in the range for lenght c..
	}
	n--; // subtracting since index starting from 0.
	ll x = n / c; // number to form
	ll y = n % c; // digit place in the number ...
