...
// ll a[mxN][mxN], b[mxN][mxN], x[mxN][mxN];
vector < vector < ll > > a(mxN, vector < ll > (mxN, INFL));
vector < vector < ll > > b(mxN, vector < ll > (mxN, INFL));
void solve(){
	ll n, m, c; cin >> n >> m >> c;
	for(int i = 1; i <= n; i++) for(int j = 1; j <= m; j++) cin >> a[i][j];
	auto calc = [&](){
		fill(all(b), vector < ll > (mxN, INFL));
		for(int i = 1; i <= n; i++){
			for(int j = 1; j <= m; j++){
				b[i][j] = min({a[i][j], b[i - 1][j] + c, b[i][j - 1] + c});
			}
		}
		ll ans = INFL;
		for(int i = 1; i <= n; i++){
			for(int j = 1; j <= m; j++){
				ans = min({ans, b[i - 1][j] + c + a[i][j], b[i][j - 1] + c + a[i][j]});
			}
		}
		return ans;
	};
