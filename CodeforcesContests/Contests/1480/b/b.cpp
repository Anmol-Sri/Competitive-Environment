/*
	Author : redarch
	Created On : 07/02/2021 20:13:17
*/
...
void solve(){
	...
	sort(all(arr));
	//sort by asc. attack power and asc. health
	for(int i = 0; i < n - 1; i++){
		ll moves = (arr[i].second + A - 1)/A;
		if(B < moves*arr[i].first){
			no(); return;
		}
		else B -= moves * arr[i].first;
	}
	ll moves = (arr[n - 1].second + A - 1)/A;
	if((moves - 1) * arr[n - 1].first <= B){
		yes();
		return;
	}
	no();
