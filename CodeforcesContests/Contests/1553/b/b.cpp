/*
	Author : redarch
	Created On : 26/07/2021 15:42:14
*/
...
void solve(){
	...
	auto recur = y_combinator([&](auto recur, ll idx, ll cur, bool dir){
		if(cur == m) return true;
		if(idx >= n || idx < 0) return false;
		if(s[idx] != t[cur]) return false;
		if(dir) if(recur(idx + 1, cur + 1, true)) return true;
		return recur(idx - 1, cur + 1, false);
	});
	bool ok = false;
	for(ll i = 0; i < n; i++) if(s[i] == t[0]){ if(recur(i, 0, true)) {print("YES"); return; }}
	print("NO");
}
