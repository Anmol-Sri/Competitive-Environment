/*
	Author : redarch
	Created On : 30/04/2021 22:11:39
*/
...
void solve(){
	ll n; cin >> n;
	vector < ll > a(n), b(n); read(a); read(b);
	ll total = 0;
	for(ll i = 0; i < n; i++) total += a[i] * b[i];
	ll ans = total;
	// taking odd length subarray and reversing it
	for(ll i = 0; i < n; i++){
		ll cur = total;
		ll l = i, r = i;
		while(l > 0 && r + 1 < n){
	...
	// taking even length subarray and reversing it
	for(ll i = 1; i < n; i++){
		ll cur = total;
		ll l = i, r = i - 1;
		while(l > 0 && r + 1 < n){
