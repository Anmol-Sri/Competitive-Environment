/*
	Author : redarch
	Created On : 30/04/2021 21:07:41
*/
...
void recur(ll x, ll y, ll cost){
    ...
    if(dp[x + 1][y] == 1 || dp[x][y + 1] == 1)  dp[x][y] = 1;
    else dp[x][y] = 0;
}
...
void solve(){
	cin >> n >> m >> k;
    if(n == m && m == 1){
        if(k == 0) print("YES"); else print("NO");
        return;
    }
    mem(dp, -1);
    recur(1, 1, 0);
    if(dp[1][1]) print("YES"); else print("NO");
}
