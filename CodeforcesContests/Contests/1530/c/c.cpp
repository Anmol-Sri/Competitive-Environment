/*
	Author : redarch
	Created On : 18/07/2021 16:47:43
*/
...
void solve(){
	ll n; cin >> n;
	vector < ll > a(n), b(n); read(a); read(b); sort(all(a), greater<>()); sort(all(b), greater<>());
	auto check = [&](ll ad){
		vector < ll > cnta(101), cntb(101);
		for(ll i = 0; i < n; i++) cnta[a[i]]++, cntb[b[i]]++;
		cnta[100] += ad;
		cntb[0] += ad;
		ll sum1 = 0, sum2 = 0;
		ll idx1 = (n + ad) - (n + ad)/4;
		ll idx2 = idx1;
		for(ll i = 100; ~i; i--){
			ll to_take = min(idx1, cnta[i]);
			sum1 += to_take * i; idx1 -= to_take;
			to_take = min(idx2, cntb[i]);
			sum2 += to_take * i; idx2 -= to_take;
		}
		return sum1 >= sum2;
	};
	ll l = -1, r = INF;
