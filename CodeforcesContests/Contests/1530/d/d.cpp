/*
	Author : redarch
	Created On : 18/07/2021 17:14:16
*/
...
void solve(){
	ll n; cin >> n;
	stack < ll > st;
	vector < ll > done(n + 1, 0), choice(n + 1), ans(n + 1), givenby(n + 1, 0);
	ll res = 0;
	for(ll i = 1; i <= n; i++){
		ll x; cin >> x;
		choice[i] = x;
	...
	for(ll i = 1; i <= n; i++) if(done[i] == 0) st.push(i);
	if(st.size() == 1){
		ll curidx = st.top();
		if(ans[curidx] == -1){
			ans[curidx] = choice[curidx];
			ans[givenby[choice[curidx]]] = curidx;
		}
	}
	ll last;
	for(ll i = 1; i <= n; i++){
