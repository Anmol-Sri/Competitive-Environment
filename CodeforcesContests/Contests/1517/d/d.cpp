/*
	Author : redarch
	Created On : 29/04/2021 17:44:39
*/
...
void solve(){
	ll h, w, k; cin >> h >> w >> k;
	vector < vector < ll > > hor(h, vector < ll > (w - 1));
	for(ll i = 0; i < h; i++){
		for(ll j = 0; j < w - 1; j++){
			cin >> hor[i][j];
		}
	}
	vector < vector < ll > > ver(h - 1, vector < ll > (w));
	for(ll i = 0; i < h - 1; i++){
		for(ll j = 0; j < w; j++){
			cin >> ver[i][j];
		}
	...
	if(k & 1){
		...
		vector < vector < ll > > new_ans(h, vector < ll > (w, 0));
		for(ll it = 0; it < k/2; it++){
			for(ll i = 0; i < h; i++){
				for(ll j = 0; j < w; j++){
					new_ans[i][j] = INFL;
					if(i > 0) chmin(new_ans[i][j], ans[i - 1][j] + 2 * ver[i - 1][j]);
					if(j > 0) chmin(new_ans[i][j], ans[i][j - 1] + 2 * hor[i][j - 1]);
					if(i < h - 1) chmin(new_ans[i][j], ans[i + 1][j] + 2 * ver[i][j]);
					if(j < w - 1) chmin(new_ans[i][j], ans[i][j + 1] + 2 * hor[i][j]);
				}
			}
			swap(ans, new_ans);
		}
