/*
	Author : redarch
	Created On : 29/04/2021 16:58:47
*/
...
bool check(ll x){
	if(x >= 0 && x < n) return true; return false;
}
...
void dfs(ll x, ll y){
	...
	arr[x][y] = val;
	cnt--;
	if(cnt > 0 && check(x) && check(y - 1) && arr[x][y - 1] == 0) dfs(x, y - 1);
	if(cnt > 0 && check(x + 1) && check(y) && arr[x + 1][y] == 0) dfs(x + 1, y);
}

void solve(){
	cin >> n;
	for(ll i = 0; i < n; i++) cin >> arr[i][i];
	for(ll i = 0; i < n; i++){
		cnt = arr[i][i];
		val = arr[i][i];
		dfs(i, i);
	}
	for(ll i = 0; i < n; i++) {for(ll j = 0; j <= i; j++) cout << arr[i][j] << ' '; print(); }
}
