/*
	Author : redarch
	Created On : 17/03/2021 16:19:19
*/
...
void solve(){
	auto query = [](ll l, ll r){ cout << "? " << l << " " << r << endl; ll x; cin >> x; return x; };
	auto ans = [](ll x){ cout << "! " << x << endl; };
	ll n; cin >> n;
	ll secmx = query(1, n);
	if(secmx == 1 || query(1, secmx) != secmx){
		ll l = secmx, r = n;
		while(r - l > 1){
			ll m = (l + r)/2;
			if(query(secmx, m) == secmx) r = m;
			else l = m;
		}
		ans(r);
	}
