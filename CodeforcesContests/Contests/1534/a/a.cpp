/*
	Author : redarch
	Created On : 14/06/2021 16:31:42
*/
...
void solve(){
	...
	for(int i = 0; i < n; i++){
		for(int j = 0; j < m; j++){
			if(s[i][j] == '.') continue;
			else if(s[i][j] != op1[i][j]){ok = false; break;}
		}
	}
	if(ok){
		print("YES");
		for(int i = 0; i < n; i++) print(op1[i]);
		return;
	}
	ok = true;
	for(int i = 0; i < n; i++){
		for(int j = 0; j < m; j++){
			if(s[i][j] == '.') continue;
			else if(s[i][j] != op2[i][j]){ ok = false; break; }
		}
	}
	if(ok){
		print("YES");
		for(int i = 0; i < n; i++) print(op2[i]);
		return;
	}
	print("NO");
}
