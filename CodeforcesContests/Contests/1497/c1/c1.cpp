/*
	Author : _reyna_
	Created On : 17/03/2021 20:56:50
*/
...
ll findlcm(vector < ll > arr, ll n){
    ll ans = arr[0];
    for (ll i = 1; i < n; i++) ans = (((arr[i] * ans)) / (__gcd(arr[i], ans)));
    return ans;
}
...
void solve(){
	...
	vector < ll > arr(3);
	arr[0] = n/3;
	if(n % 3 == 1) arr[1] = n/3, arr[2] = n/3 + 1;
	else if(n % 3 == 2) arr[1] = n/3, arr[2] = n/3 + 2;
	else arr[1] = n/3, arr[2] = n/3;

	while(findlcm(arr, 3) <= n/2){
