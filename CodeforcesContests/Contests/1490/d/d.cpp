/*
	Author : redarch
	Created On : 11/03/2021 17:17:03
*/
...
vector < int > ans;
void recur(vector < int > arr, int start, int end, int prev){
	// print(start, end);
	if(start > end) return;
	if(start == end){
		ans[start] = prev + 1;
	...
	if(start == end - 1){
		if(arr[start] < arr[end]) ans[start] = prev + 1, ans[end] = prev + 2;
		else ans[start] = prev + 2, ans[end] = prev + 1;
	}
	int mxi = start;
	...
	ans[mxi] = prev + 1;
	if(mxi - 1 >= start) recur(arr, start, mxi - 1, prev + 1);
	if(mxi + 1 <= end) recur(arr, mxi + 1, end, prev + 1);
}
/*
3 5 2 1 4
0 1 2 3 4
