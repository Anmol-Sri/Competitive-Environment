/*
	Author : redarch
	Created On : 03/06/2021 21:13:08
*/
...
void solve(){
	...
	vector < ll > rea(n);
	iota(all(rea), 0);
	sort(all(rea), [&](ll x, ll y){
		return a[x].first < a[y].first;
	});
	vector < ll > ans(n, -1LL);
	vector < vector < ll > > par(2);
	for(auto i : rea){
		ll p = a[i].first % 2;
		if(a[i].second == -1){
			if(par[p].empty()) par[p].pb(i);
			else{
				ll j = par[p].back();
				par[p].pop_back();
	...
	for(ll p = 0; p < 2; p++){
		while(sz(par[p]) > 1){
			ll i = par[p].back(); par[p].pop_back();
			ll j = par[p].back(); par[p].pop_back();
