/*
	Author : redarch
	Created On : 07/06/2021 14:33:19
*/
...
ll recur(ll i, ll j){ // i -> full chairs, j -> emt chairs
	if(i >= full.size()) return 0;
	if(j >= emt.size()) return INFL;
	if(dp[i][j] != -1) return dp[i][j];
	dp[i][j] = min(abs(full[i] - emt[j]) + recur(i + 1, j + 1), recur(i, j + 1));
	return dp[i][j];
}
...
void solve(){
	...
	a.resize(n); read(a);
	for(ll i = 0; i < n; i++){
		if(a[i]) full.pb(i); else emt.pb(i);
	}
	print(recur(0, 0));
}
