/*
	Author : redarch
	Created On : 03/05/2021 23:53:35
*/
...
void solve(){
	...
	if(n == 1){
		if(a[0] == x) print("NO"); else {print("YES"); db(a);} return;
	}
	auto check = [&](){
		ll sum = 0;
		for(ll i = 0; i < n; i++){
	...
	while(idx != -1 && cnt <= n){
		...
		cnt++;
		if(cnt > n) break;
		if(idx < n - 1) swap(a[idx], a[n - 1]); else swap(a[idx], a[idx - 1]);
	}
	if(idx == -1) {print("YES"); db(a); } else print("NO");
}
