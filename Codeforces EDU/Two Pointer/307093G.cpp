...
struct stack{
	vector < ll > s, sgcd = {0};
	void push(ll x){
		s.pb(x); sgcd.pb(__gcd(sgcd.back(), x));
	}
	ll pop(){
		ll res = s.back();
		s.pop_back(); sgcd.pop_back();
		return res;
	}
	bool empty(){ return s.empty(); }
	ll cg(){ return sgcd.back(); }
} s1, s2;
...
bool good(){
	ll fir = s1.cg(); ll sec = s2.cg();
	return __gcd(fir, sec) == 1;
}

void solve(){
    cin >> n; vector < ll > arr(n); read(arr);
