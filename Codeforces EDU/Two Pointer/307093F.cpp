...
struct stack{
	...
	ll pop(){
		ll res = s.back();
		s.pop_back(); smin.pop_back(); smax.pop_back();
		return res;
	}
	bool empty(){ return s.empty(); }
	ll cmin(){ return smin.back(); }
	ll cmax(){ return smax.back(); }
} s1, s2;
...
bool good(){
	ll mn = min(s1.cmin(), s2.cmin());
	ll mx = max(s1.cmax(), s2.cmax());
	return mx - mn <= k;
}

void solve(){
	cin >> n >> k; vector < ll > arr(n); read(arr);
	ll l = 0, res = 0;
	for(ll r = 0; r < n; r++){
		add(arr[r]);
		while(!good()) remove(), l++;
