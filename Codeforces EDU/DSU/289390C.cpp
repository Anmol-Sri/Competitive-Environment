...
struct UnionFind {
	...
	UnionFind(ll N){
		...
		setSize.resize(N);
        sum.resize(N);
		for (ll i = 0; i < N; i++) {  p[i] = i; setSize[i] = 1; sum[i] = 0;}
	}
	int findSet(ll i) { while(i != p[i]) i = p[i]; return p[i]; }
	...
	void unionSet(ll i, ll j){ 
        ...
        if(x == y) return; 
		numSets--; 
		if(rank[x] > rank[y]) { sum[y] -= sum[x]; p[y] = x; setSize[x] += setSize[y];}
		else{
            sum[x] -= sum[y];
    ...
    void addToSet(ll a, ll b){
        a = findSet(a);
        sum[a] += b;
    }
    ll getSum(ll a){ ll ans = 0; while(a != p[a]){ ans += sum[a]; a = p[a]; } return ans + sum[a]; }
...
void solve(){
    ...
    while(m--){
        string s; cin >> s;
        if(s == "join"){ ll a, b; cin >> a >> b; u.unionSet(a, b); }
        else if(s == "add"){ll a, b; cin >> a >> b; u.addToSet(a, b); }
        else{ll a; cin >> a; print(u.getSum(a)); }
    }
}
