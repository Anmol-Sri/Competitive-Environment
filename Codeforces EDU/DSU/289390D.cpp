...
void solve(){
    ...
    for(auto x : mp) if(x.second > 0) u.unionSet(x.first.first, x.first.second), print("union of : ", x.first.first, x.first.second);
    reverse(all(q));
    for(auto x : q){
        if(x[2] == 1) ans.pb(u.isSameSet(x[0], x[1]) ? "YES" : "NO");
        else u.unionSet(x[0], x[1]);
    }
    reverse(all(ans));
