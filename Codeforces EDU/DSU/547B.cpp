...
void solve(){
    UnionFind u(mxN);
    ll n; cin >> n;
    for(ll i = 0; i < n; i++){cin >> p[i].first; p[i].second = i;}
    ...
    for(ll i = 0; i < n; i++){
        ...
        ll val = p[i].first;
        vis[pos] = true;
        if(pos && vis[pos - 1]) u.unionSet(pos, pos - 1);
        if(pos < n - 1 && vis[pos + 1]) u.unionSet(pos, pos + 1);
        ll siz = u.sizeOfSet(u.findSet(pos));
        while(cur <= siz) ans[cur++] = val;
    }
