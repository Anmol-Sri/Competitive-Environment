...
void solve(){
	string s; cin >> s;
	ll n = sz(s);
	vector < ll > pref = prefix_function(s);
	if(pref[n - 1] == 0) {print("Just a legend"); return; }
	for(int i = 1; i < n - 1; i++){
		if(pref[i] == pref[n - 1]){
			print(s.substr(n - pref[n - 1], pref[i]));
			return ;
		}
	}
	if(pref[pref[n - 1] - 1] == 0){
		print("Just a legend"); return;
	}
	print(s.substr(0, pref[pref[n - 1] - 1]));
}
