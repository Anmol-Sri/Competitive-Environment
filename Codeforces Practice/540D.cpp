	...
	for(ll sum = R + S + P; sum; sum--){
		for(ll r = R; ~r; r--){
			for(ll s = S; ~s; s--){
				...
				if(s == 0 && p == 0) continue;
				ld cur = dp[r][s][p];
				ll wayr = r * s, ways = s * p, wayp = p * r;
				ll total = wayr + ways + wayp;
				if(r > 0) dp[r - 1][s][p] += cur * wayp/total;
				if(s > 0) dp[r][s - 1][p] += cur * wayr/total;
				if(p > 0) dp[r][s][p - 1] += cur * ways/total;
			}
		}
	}
	ld ansr = 0, anss = 0, ansp = 0;
	for(ll r = 1; r <= R; r++) ansr += dp[r][0][0];
	for(ll s = 1; s <= S; s++) anss += dp[0][s][0];
	for(ll p = 1; p <= P; p++) ansp += dp[0][0][p];
	cout << ansr << ' ' << anss << ' ' << ansp << nl;
}
