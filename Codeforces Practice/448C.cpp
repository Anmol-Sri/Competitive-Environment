...
ll recur(ll l, ll r){
    ll mn = INFL;
    for(ll i = l; i <= r; i++) mn = min(mn, arr[i]);
    ll cur = mn;
    for(ll i = l; i <= r; i++) arr[i] -= mn;
    ll start = l;
    for(ll i = l; i <= r; i++){
        if(arr[i] == 0){
            if(start < i) cur += recur(start, i - 1);
            start = i + 1;
        }
    }
    if(start <= r) cur += recur(start, r);
...
void solve(){
    ...
    for(ll i = 0; i < n; i++){
        if(arr[i] == 0){
            if(start < i) ans += recur(start, i - 1);
            start = i + 1;
        }
    }
    if(start < n) ans += recur(start, n - 1);
    print(min(ans, n));
}
