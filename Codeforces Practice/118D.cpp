...
ll recur(ll rem1, ll rem2, ll c1, ll c2){
	if(rem1 == 0 && rem2 == 0) return 1;
	ll cur = dp[rem1][rem2][c1][c2];
	if(cur != INFL) return cur;
	cur = 0;
	if(rem1 && c1 < k1) cur = (cur + recur(rem1 - 1, rem2, c1 + 1, 0)) % MOD3;
	if(rem2 && c2 < k2) cur = (cur + recur(rem1, rem2 - 1, 0, c2 + 1)) % MOD3;
	return dp[rem1][rem2][c1][c2] = cur;
}
