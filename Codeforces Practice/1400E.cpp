...
void solve(){
    ll n; cin >> n; vector < ll > a(n); read(a);
    auto ans = y_combinator([&](auto recur, ll l, ll r, ll h) -> ll{
        if(l == r) return 0;
        ll vc = (r - l); 
        ll m = (int)(min_element(a.begin() + l, a.begin() + r) - a.begin());
        ll hc = recur(l, m, a[m]) + recur(m + 1, r, a[m]) + (a[m] - h);
        return min(vc, hc);
    })(0, n, 0);
    print(ans);
}
