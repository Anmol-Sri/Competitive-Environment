...
void solve(){
	ll n, c; cin >> n >> c;
    vector < ll > a(n), b(n - 1); read(a); read(b);
    ll ans = INFL;
    ll day[n]; // days passed till i 
    ll bal[n]; // amount collected till i
    day[0] = bal[0] = 0; // days needed to be on 0 and balance on day 0
    for(ll i = 0; i < n; i++){
        ans = min(ans, day[i] + max(0LL, c - bal[i] + a[i] - 1)/a[i]);
        if(i == n - 1) break;
        ll min_day_needed_for_jump = max(0LL, b[i] - bal[i] + a[i] - 1)/a[i];
        day[i + 1] = day[i] + min_day_needed_for_jump + 1;
        bal[i + 1] = bal[i] + min_day_needed_for_jump * a[i] - b[i];
    }
    print(ans);
}
