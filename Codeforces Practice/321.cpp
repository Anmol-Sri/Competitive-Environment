...
vector < pair < int, bool > > ch[mxN];

int dfs(int v, int x, int a, int  b){
	int res = max(0, (a - b + 1) / 2);
	for(auto i : ch[v]){
		if(!i.second){
			int temp = dfs(i.first, x + 1, a + 1, b);
			if(temp == x + 1) ans.pb(MP(i.first, v)), res = max(res, max(0, temp - 1));
			else res = max(res, temp);
		}
		else{
			res = max(res, dfs(i.first, x, a, b + 1));
		}
	}
	return res;
}
