            ...
            if(s[i][j]=='1'){
                ...
                for(ll i1 = i; i1 < n; i1++){
                    for(ll j1 = j; j1 < m; j1++){
                        ...
                        if(j1!=j){
                            if(s[i1][j1-1]=='1' || s[i1][j1-1]=='.'){
                                if(i1==n-1){
                                    v[i1][j1] += v[i1][j1-1];
                                }
                                else{
                                    v[i1][j1] += (v[i1][j1-1]*x*iv)%MOD;
                                }
                            }
                        }
                        v[i1][j1]%=MOD;
                        if(i1!=i){
                            if(s[i1-1][j1]=='1' || s[i1-1][j1]=='.'){
                                if(j1==m-1){
                                    v[i1][j1] += v[i1-1][j1];
                                }
                                else{
                                    v[i1][j1] += (((v[i1-1][j1]*(y-x))%MOD)*iv)%MOD;
                                }
                            }
                        }
                        v[i1][j1]%=MOD;
                ...
                ans += tmp*powermod(1-tmp1, MOD-2LL, MOD);
                ans %= MOD;
            }
        }
    }

    ans *= powermod(p, MOD-2, MOD);
    ans%=MOD;
    ans+=MOD;
    ans%=MOD;
    print(ans);
