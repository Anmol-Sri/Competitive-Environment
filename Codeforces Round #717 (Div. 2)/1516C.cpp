...
void solve(){
    ...
    vector < ll > copy = arr;
    ll sum = 0;
    vector < ll > pos(2001, -1);
    for(ll i = 0; i < n; i++){
        sum += arr[i];
        pos[arr[i]] = i;
    }
    sort(all(arr));
    if(sum % 2 != 0){ print(0); return; }
    auto calc = y_combinator([&](auto calc, ll q, ll cur) -> bool {
        if(cur == 0) return true;
        if(q == 0 && cur != 0) return false;
        if(arr[q - 1] > cur) return calc(q - 1, cur);
        return calc(q - 1, cur) || calc(q - 1, cur - arr[q - 1]);
    });
    if(calc(n, sum) == false){
        print(0); return;
    }
    ...
    //     arr.erase(it);
    //     db(arr);
    //     if(calc(n - 1, sum - temp) == false){
    //         print(1); print(pos[temp] + 1); return;
    //     }
