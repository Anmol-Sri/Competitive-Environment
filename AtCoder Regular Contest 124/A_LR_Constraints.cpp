...
const ll LIM = 1111; // change the limit according to the question
...
void solve(){
	ll n, k; cin >> n >> k;
    ll ans = 0;
    for(ll i = 1; i <= k; i++) ans = add(ans, mult(mult(power(-1, i + k), nCr(k, i)), powermod(i, n, _MOD_)));
    print(ans);
    // ans = mult(n, k);
    ll temp = 0;
    for(ll i = 1; i <= k; i++){
        ...
        if(ch == 'L'){
            // ans = sub(ans, mult(k, x));
            temp = add(temp, n - x + 1);
        }
        else{
            temp = mult(temp, sub(ans, n - x));
        }
    }
    ans = sub(ans, temp);
    print();
