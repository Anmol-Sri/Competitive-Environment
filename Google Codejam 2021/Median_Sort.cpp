...
void solve(){
    vector < ll > arr;
    ll c = 0; vector < ll > fir;
    print(1, 2, 3); ll cnt = 3;
    ll check; cin >> check;
    auto calc1 = [&](){
        ll sum = 0, cnt = 1;
        vector < ll > temp1, temp2; 
        if(c < n - 1) return temp1;
        for(ll i = n - 1; i; i--){
            cnt++;
            if(c <= sum + cnt + i - 1){
                ll check = c - (sum + i - 1);
                temp2.push_back(check);
                for(ll j = 0; j < i - 1; j++) temp2.push_back(1);
                sum = c;
                break;
            }
            sum += cnt;
        ...
        if(sum < c) return temp1;
        return temp2;
    };
    if(check == 3) arr = {1, 3, 2};
    else if(check == 2) arr = {1, 2, 3};
    else arr = {2, 1, 3};
    auto calc2 = [&](){
        vector < ll > temp1, store = arr;
        for(ll i = 0; i < fir.size(); i++){
            for(ll j = sz(store) - i - 2; j < sz(store) - i - 2 + fir[i]; j++) temp1.push_back(store[j]);
            reverse(all(temp1));
            for(ll j = sz(store) - i - 2; j < sz(store) - i - 2 + fir[i]; j++) store[j] = temp1[j - sz(store) + i + 2];
            temp1.clear();
        }
        return store;
    };
    for(ll i = 4; i <= n; i++){
        ...
        while(l < r){
            ll m = (l + r) / 2;
            print(arr[m], arr[m + 1], i);
            cin >> check;
            if(check == arr[m + 1]) l = m + 1; else if(check == arr[m]) r = m; else{ arr.insert(arr.begin() + m + 1, i); cnt++; break;}
        }
        auto calc = [](string s, string r, string t){
            string ns = "";
            for(ll i = 0; i < s.size() - 1; i++){
                string check = to_string(s[i]) + to_string(s[i + 1]);
                if(check == r){
                    s[i] = t[0];
                    s[i + 1] = t[1];
                }
        ...
        if(cnt != i){
            if(l == 0) arr.insert(arr.begin(), i);
            else arr.pb(i);
            cnt++;
        }
    }
    for(ll i = 0; i < arr.size(); i++) cout << arr[i] << ' '; print();
    cin >> check; if(check == -1) return;
}
