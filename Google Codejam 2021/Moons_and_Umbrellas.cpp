...
void solve(){
    ...
    if(ques == n - 1 || ques == n){ print(0); return; }
    ll fir = 0;
    auto calcfordif = [](){
        ll n; cin >> n; vector < ll > arr(n); read(arr);
        ll ans = 0;
        for(ll i = 0; i < n - 1; i++){
            ll mni = i;
            for(ll j = i + 1; j < n; j++){
    ...
    for(ll i = 0; i < n - 1; i++) if(s[i] != '?'){fir = i; break; }
    if(fir == 0) fir++;
    ll sec = fir;
    for(; sec < n - 1; sec++){
        // if(x > 0 && y < 0){
    ...
    // }
            if(s[sec + 1] == '?'){ 
                ...
                //     }
                if(temp == n) break;
                if(s[sec - 1] != s[temp]) if(s[sec - 1] == 'J') res += y; else res += x;
                sec = temp;
            }
            else if(s[sec + 1] != '?'){
                if(s[sec - 1] == 'J') res += y;
    ...
    // // print(ns);
    // ll ans = 0;
    // for(ll i= 0; i < ns.size() - 1; i++){
    //     string check = to_string(ns[i]) + to_string(ns[i + 1]);
    //     // dbvar(check);
    //     if(check == "JC") ans += y;
    //     else if(check == "CJ") ans += x;
    // }
    // print(ans);
    // for(ll i = 0; i < s.size() - 1; i++){
    //     string check = to_string(ns[i]) + to_string(ns[i + 1]);
    //     if(check == "JC") ans += y; else if(check == "CJ") ans += x;
    // }
    // print(ans);
    // ll x, y; string s; cin >> x >> y >> s;
    // ll ans = 0; string ns = "";
