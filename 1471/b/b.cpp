/*
	...
	Created On : 05/01/2021 20:12:46
*/
...
void solve(){
	...
	for(int i = 0; i < n; i++){
		cin >> arr[i].second; arr[i].first = 1;
	}
	ll sum = 0; bool ok = true;
	for(int i = 0; i < sz(arr); i++){
		if(arr[i].second % x != 0) break;
		int temp = (arr[i].second) / x;
		arr.pb({arr[i].first * x, temp});
	}
	for(int i = 0; i < sz(arr); i++) sum += arr[i].second * arr[i].first;
	print(sum);
}
