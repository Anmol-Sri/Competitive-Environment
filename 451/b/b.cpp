/*
	Author : redarch
	Created On : 13/01/2021 22:31:18
*/
...
void solve(){
	...
	sort(all(copy));
	for(int i = 0; i < n; i++) mp[copy[i]] = i;
	for(int i = 0; i < n; i++) arr[i] = mp[arr[i]];
	int l = -1, r = -1;
	for(int i = 0; i < n; i++) if(arr[i] != i){l = i; break; }
	for(int i = n - 1; ~i; i--) if(arr[i] != i){r = i; break; }
	if(l == -1 || r == -1){
		print("yes");
		print(1, 1);
		return;
	...
	bool ok = true;
	for(int i = 0; i < n; i++) if(arr[i] != i){ ok = false; break; }
	if(ok){ print("yes"); print(++l, ++r);} else print("no");
}
