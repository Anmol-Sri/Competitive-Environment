/*
	Author : redarch
	Created On : 15/01/2021 16:43:16
*/
...
void solve(){
	...
	for(int i = 0; i < s.size(); i++){
		if(s[i] == '@') cnt++;
		if((i == 0 || i == s.size() - 1) && s[i] == '@') ok = false;
		if(i < s.size() - 1 && s[i] == '@' && s[i + 1] == '@') ok = false;
		if(ok  && s[i] == '@' && i != s.size() - 1){
			if(i + 2 < s.size() && (s[i + 1] == '@' || s[i + 2] == '@')) ok = false;
		}
	}
	if(!ok || cnt == 0){ print("No solution"); return; }
	else{
		for(int i = 0; i < s.size(); i++){
			cout << s[i];
			if(s[i] == '@'){
