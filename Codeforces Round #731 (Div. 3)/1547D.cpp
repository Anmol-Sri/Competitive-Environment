...
void solve(){
    ...
    b[0] = 0;
    for(ll i = 1; i < n; i++){
        if((a[i - 1] & a[i]) == a[i - 1]) b[i] = 0;
        else{
            bitset < 31 > b1(a[i - 1]), b2(a[i]);
            string s1 = b1.to_string(), s2 = b2.to_string();
            string need = "";
            ll val = -1;
            for(ll j = 0; j < 31; j++) need += '0';
            for(ll j = 30; ~j; j--){
                if(s1[j] == s2[j]) need[j] = s1[j];
                else need[j] = s2[j];
                bitset < 31 > b3(need);
                ll temp = b3.to_ullong();
    ...
    // for(ll i = 1; i < n; i++){
    //     if((a[i] & a[i - 1]) != a[i - 1]) print(i, a[i] & a[i - 1]);
    // }
    db(b);
}
