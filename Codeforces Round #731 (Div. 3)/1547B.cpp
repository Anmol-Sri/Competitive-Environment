...
string s;
bool recur(ll l, ll r, char ch){
    if(l == 0 && r == s.size() - 1) return true;
    if(s[l - 1] == (char)(ch + 1)) return recur(l - 1, r, (char)(ch + 1));
    else if(s[r + 1] == (char)(ch + 1)) return recur(l, r + 1, (char)(ch + 1));
    return false;
}
void solve(){
	cin >> s;
    bool ok = false; ll pos = -1;
    for(ll i = 0; i < s.size(); i++) if(s[i] == 'a') {ok = true; pos = i;};
    if(pos == -1) print("NO");
    else if(recur(pos, pos, 'a')) print("YES");
    else print("NO");
}
