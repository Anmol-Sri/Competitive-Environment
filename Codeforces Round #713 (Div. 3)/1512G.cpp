...
{	
    ...
    for(ll i = 1; i < mxN; i++){
        ans[i] = INFL;
        for(ll j = i; j < mxN; j += i) sum[j] += i;
    }
    // sum stores the sum of divisors of a number
    // for eg. sum[2] = 1 + 2 = 3
    // sum[3] = 1 + 3 = 4
    // sum[4] = 1 + 2 + 4 = 7
    // sum[5] = 1 + 5 = 6 and so on...
    // ans will store the ans for eg. 
    // if for i = 4 : sum[4] = 7...
    // then ans[7] = min(ans[7], 4)
    for(ll i = 1; i < mxN; i++) if(sum[i] < mxN && ans[sum[i]] == INFL) ans[sum[i]] = i;
	int t = 1;
	cin >> t;
