...
void solve(){
    ll n; cin >> n; vector < pair < ll, ll > > pos;
    char arr[n][n];
    for(ll i = 0; i < n; i++) for(ll j = 0; j < n; j++){
        cin >> arr[i][j];
        if(arr[i][j] == '*') pos.pb(MP(i, j));
    }
    ll x1 = pos[0].first, y1 = pos[0].second;
    ll x2 = pos[1].first, y2 = pos[1].second;
    ll x3 = min(x1, x2), y3 = min(y1, y2);
    ll x4 = max(x1, x2), y4 = max(y1, y2);
    // if(x3 == x)
    if(x1 == x2){
        if(x1 == 0) x3 = 1, x4 = 1;
        else if(x1 == n - 1) x3 = n - 2, x4 = n - 2;
        else x3++, x4++;
    }
    if(y1 == y2){
        if(y1 == 0) y3 = 1, y4 = 1;
        else if(y1 == n - 1) y3 = n - 2, y4 = n - 2;
        else y3++, y4++;
    }
    if(x3 == x1 && y3 == y1) swap(x3, x4);
    for(ll i = 0; i < n; i++){
        for(ll j = 0; j < n; j++){
            if(i == x3 && j == y3) cout << '*';
