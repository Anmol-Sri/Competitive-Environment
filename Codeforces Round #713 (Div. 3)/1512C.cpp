...
void solve(){
   ...
   for(ll i = 0; i < n/2; i++){
       ...
       if(s[i] == '1') one -= 2; else if(s[i] == '0') zero -= 2;
   }
   if(n % 2){
	   if(s[n/2] == '0') zero--;
	   else if(s[n/2] == '1')  one--;
   }
   if(one < 0 || zero < 0){print(-1); return; }
   for(ll i = 0; i < n/2; i++){
       if(s[i] == s[n - 1 - i] && s[i] == '-'){
           if(one >= 2){ s[i] = '1'; s[n - 1 - i] = '1'; one -= 2;}
           else if(zero >= 2) {s[i] = '0'; s[n - 1 - i] = '0'; zero -= 2; }
           else{
                // print(-2, s, one, zero); return;
                print(-1); return;
           }
       }
   }
