...
void solve(){
	...
	vector < ll > ans(n, 0), used(n + 1, 0);
	ll len = r - l + 1;
	auto conc = [](ll x){return (x * (x + 1))/2; };
	ll mn = conc(len);
	ll mx = conc(n) - conc(n - len);
	ll cur = n;
	if(s < mn || s > mx){print(-1); return; }
	ll pos = l - 1;
	while(s){
		// check whether u can put min of n or sum - consecutive sum of len - 1 
		// for eg. if len = 3 then u can put 1, 2, 3 or directly start by n
		ll ele = min(cur, s - conc(len - 1)); 
		ans[pos] = ele;
		cur = ele - 1;
		used[ele] = 1;
		s -= ele;
	...
	cur = 1;
	for(ll i = 0; i < n; i++) if(l - 1 <= i && i <= r - 1) continue; else{while(used[cur]) cur++; ans[i] = cur; cur++;}
	db(ans);
}
