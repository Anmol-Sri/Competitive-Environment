/*
	Author : redarch
	Created On : 14/01/2021 20:13:04
*/
...
void solve(){
	...
	auto calc = [&](string a){
		for(int j = 0; j < a.size(); j++){
			string test = a.substr(0, j + 1);
			// print(test);
			int iter = test.size();
			string add = test;
			for(int i = iter; i < a.size(); i += iter, test += add);
			// print("full lenght = ", test, "with a = ", a);
			if(test == a){
				// print(" returning ans = ", add);
				return add;
			}
		}
		string ans = "";
	...
	else if(newa != "" && newa == newb){
		int len = (a.size() * b.size())/__gcd(a.size(), b.size());
		cout << newa;
		for(int i = newa.size(); i < len; i += newa.size()) cout << newa;
		print();
	}
	else print(-1);
