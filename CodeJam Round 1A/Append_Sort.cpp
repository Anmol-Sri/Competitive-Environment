...
void solve(){
    ...
    for(ll i = 1; i < n; i++){
        ...
        for(ll j = 0; j < arr[i].size(); j++){
            ll fir = arr[i][j] - '0';
            ll sec = arr[i - 1][j] - '0';
            if(fir > sec){ok = true; break;}
            else if(fir == sec) continue;
            else break;
        }
        if(ok){
            ll x =  arr[i - 1].size() - arr[i].size();
            ans += x;
            for(ll j = arr[i].size(); j < arr[i - 1].size(); j++) arr[i] += '0';
        }
        else{
            // dbvar("check top = ", ans);
            ans += arr[i - 1].size() - arr[i].size();
            ll cnt = arr[i - 1].size() - arr[i].size();
            // dbvar("check middle 1= ", ans, cnt, arr[i], arr[i - 1]);
            // print(i, "check for arr[i] : ", arr[i]);
            for(ll j = arr[i].size(); j < arr[i - 1].size(); j++) arr[i] += arr[i - 1][j];
            if(stoll(arr[i]) < stoll(arr[i - 1])) {arr[i] += '0'; ans++;}
            // dbvar("check middle 2= ", ans);
            if(stoll(arr[i]) > stoll(arr[i - 1])) continue;
            ll pos;
            for(pos = arr[i].size() - 1; pos >= 0; --pos){
                if(arr[i][pos] != '9') break;
            }
            if(pos == -1){
                ans++;
                arr[i] += '0';
            }
            else if(arr[i].size() - pos > cnt){
                ans++;
                arr[i] += '0';
            }
            else{
                // dbvar("check = ", ans);
                arr[i][pos] = ((arr[i][pos] - '0') + 1) + '0';
                for(pos++; pos < arr[i].size(); pos++) arr[i][pos] = '0';
            }
        }
        // dbvar(i,ans);
    }
