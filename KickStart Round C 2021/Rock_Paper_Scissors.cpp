...
const int mxN = 66;
char best[mxN][mxN][mxN];
double dp[mxN][mxN][mxN];
bool vis[mxN][mxN][mxN];
ll w, e;

ll recur(ll day, ll r, ll s){
    if(day == 60) return 0;
    if(vis[day][r][s]) return dp[day][r][s];
    vis[day][r][s] = true;
    ll p = day - r - s;
    double pr, pp, ps;
    if(day == 0){
    ...
    double ans = 0, tmp = 0;
    //rock 
    tmp = recur(day + 1, r + 1, s) + ps * w + pr * e;
    if(tmp > ans){ans = tmp; best[day][r][s] = 'R'; }
    //scissor 
    tmp = recur(day + 1, r, s + 1) + pp * w + ps * e;
    if(tmp > ans){ans = tmp; best[day][r][s] = 'S'; }
    //paper
    tmp = recur(day + 1, r, s) + pr * w + pp * e;
    if(tmp > ans){ans = tmp; best[day][r][s] = 'P'; }
    return dp[day][r][s] = ans;
}
...
void solve(){
    ...
    cin >> w >> e;
    recur(0, 0, 0);
    ll r = 0, s = 0; string ans = "";
    for(ll day = 0; day < 60; day++){
        char ch = best[day][r][s];
        ans += ch;
        if(ch == 'R') r++;
        else if(ch == 'S') s++;
    }
