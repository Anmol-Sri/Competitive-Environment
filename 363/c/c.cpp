/*
	Author : redarch
	Created On : 20/01/2021 21:15:39
*/
...
void solve(){
	...
	for(int i = 0; i < s.size(); i++){
		bool ok = false;
		if(ans.size() > 1 && ans[ans.size() - 1] == ans[ans.size() - 2] && ans[ans.size() - 1] == s[i]) ok = true;
		if(ans.size() > 2 && ans[ans.size() - 1] == s[i] && ans[ans.size() - 2] == ans[ans.size() - 3]) ok = true;
		if(!ok) ans += s[i];
	}
	print(ans);
}
